...
Studio keeps those follow controls compact, adds hover explanations for each mode, writes the active follow mode into the hash for deep-linking, and uses `_details` ETag long polling while a stream is actively following so live and tail updates arrive without the old 100ms metadata poll loop.
...
Older filtered pages also stop recomputing exact totals on every paginated `_search` request. Studio keeps the exact total from the leading page when it already has it, then lets follow-on pages use the cheaper request shape so load-more stays responsive on busy streams.
While a search is active, the footer summary switches from `events + bytes` to search progress, showing how many matching rows are currently loaded plus how far Studio has scanned back through the stream to find them. Once the filtered result set is exhausted, that progress now resolves to the full stream size so the footer and the `Reached the beginning of the stream` message agree instead of undercounting the last unmatched tail. That same summary box adds a subtle fill proportional to scanned coverage, and only the user-triggered infinite-scroll fetch path adds a brief neutral loading pulse so background follow-mode refreshes do not look like manual pagination work.
...
Aggregation refresh now follows the main stream mode instead of a separate toggle: `Paused` freezes the metrics band, while `Live` and `Tail` keep both the event log and the aggregation strip current.
